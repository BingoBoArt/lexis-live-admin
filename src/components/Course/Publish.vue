      ...
      <div class="form-table-box">
        <el-table type="index" v-loading="loading" :data="resultList" style="width: 100%" min-width="100px" border stripe highlight-current-row>
          ...
          <el-table-column align="center" prop="status" label="Status" width="100">
            <!--status：1  显示已上架   btn下架-->
            <!--status:   2  显示 准备上架    btn上架-->
            <!--status：3  显示已下架  btn上架-->
            <template slot-scope="scope">
              <p v-if="scope.row.status == 1">已上架</p>
              <p v-else-if="scope.row.status == 2">准备上架</p>
              <p v-else-if="scope.row.status == 3">已下架</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="lessonNo" label="Action" width="120">
            <template slot-scope="scope">
              <!--<el-button type="warning" size="small" icon="el-icon-upload" style="font-weight: 200;" v-if="scope.row.status == 1" @click="publishLesson(scope,3)">下架</el-button>-->
              <el-button type="info" size="small" style="font-weight: 200;" disabled v-if="scope.row.status == 1">已上架</el-button>
              <el-button type="warning" size="small" icon="el-icon-upload" style="font-weight: 200;" v-if="scope.row.status == 2||scope.row.status == 3" @click="publishLesson(scope,1)">上架</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="page-box">
          <el-pagination @current-change="handlePageChange" :page-size="page.rows" :total="page.total" :current-page="page.page" background layout="total, prev, pager, next, jumper">
          </el-pagination>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    publishLesson(scope, status) {
      const index = scope.$index;
      const self = this;
      Request({
        type: 'post',
        // url: '/lesson/updateLessonReleaseStatus',
        url: '/lesson/onShelf',
        data: {
        ...
        if (res.code == 0) {
          self.resultList[index].status = status;
          self.$message({
            type: 'success',
            message: '上架成功!',
            duration: 2000,
    ...
    getLessonList() {
      ...
      let self = this;
      Request({
        url: '/lesson/getLessonReleaseListPage',
        data: {
          ...this.selectCon,
          page: self.page.page,
          rows: self.page.rows,
        ...
        this.resultList = res.data.lessonReleaseList;
